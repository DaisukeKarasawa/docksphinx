...
- ネットワーク（送受信バイト数）
- ボリューム（使用量）
  - **MVP固定判断**：Docker Engine APIのみではポータブルに厳密取得が難しいため、MVPでは代替指標（mount件数、name/source/destination/driver等のmetadata）を採用する
  - UI上は `N/A (metadata-only)` と明示する
...
- `mem_threshold`：メモリ閾値超過（**連続N回**で確定）
  - **MVP運用最適化**：同一イベントの連投抑制のため、コンテナ×イベント種別単位で最小クールダウンを許容する

### TUI要件

#### レイアウト（複数ペイン）

- **左ペイン**：対象（コンテナ/イメージ/ネットワーク/ボリューム/フィルタ）
- **中央ペイン**：一覧（メトリクス/イベント）
...
- セキュリティ既定（MVP）
  - gRPC bind は原則 `127.0.0.1`（外部公開しない）
  - gRPC reflection はデフォルト無効（必要時のみ設定で有効）
  - 設定値（正規表現、パス、閾値、間隔）はバリデーションして異常値で安全に失敗する

### メトリクス定義（MVP固定）

- CPU%：Docker stats の `cpuDelta/systemDelta * numCPU * 100`
- Memory%：`usage/limit * 100`（limit欠損時は `N/A`）
- Network Rx/Tx：インタフェース合算
- Uptime：`StartedAt` 起点（parse不可時は `N/A`）
- Volume usage：上記のMVP代替指標を採用

### 依存関係可視化のMVP方針

- Docker Compose由来のまとまりはラベル優先で推定する
  - `com.docker.compose.project`
  - `com.docker.compose.service`
- ラベル欠損時はネットワーク接続からヒューリスティックに推定する
- 厳密な依存関係解析はMVP対象外
